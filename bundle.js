...
var lineChart = function () {
  ...
  _createClass(lineChart, [{
    ...
    value: function render(key) {
      ...
      var svg = d3.select(".line-chart-index-item-" + key).append("svg").attr("width", 700).attr("height", 250);
      var margin = { top: 20, right: 70, bottom: 30, left: 40 },
          width = +svg.attr("width") - margin.left - margin.right,
          height = +svg.attr("height") - margin.top - margin.bottom;

      var item = d3.select(".line-chart-index-item-" + key);
      var lineItem = d3.select(".line-chart-" + key);
      ...
      d3.json("./data/history/" + key + "_history.json", function (error, data) {
        ...
        var x = d3.scaleTime().domain([data[0].timestamp, data[data.length - 1].timestamp]).range([0, width]);
        ...
        // y-axis line
        g.append("g").attr("class", "axis axis--y").call(d3.axisLeft(y).ticks(5).tickFormat(function (d) {
          return parseInt(d);
        })).append("text").attr("class", "axis-title").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").attr("fill", "#5D6971").text("(Value)");
...
var _ranking = __webpack_require__(174);

var _line_chart = __webpack_require__(90);

var _line_chart2 = _interopRequireDefault(_line_chart);

var _bubble_chart = __webpack_require__(466);
    ...
    value: function render(data) {
      ...
      function ticked() {
        ...
        u.enter().append('circle').attr('r', function (d) {
          ...
          tooltip.html("<br/>" + d.name + "<br/>" + "Ticker: " + d.ticker + "<br/>" + "Rank: " + d.rank + "<br/>" + "Value(usd): $" + d.usd + "<br/>").style("opacity", "1").style("left", d.x + 20 + "px").style("top", d.y + "px");
        }).on("mouseout", function () {
